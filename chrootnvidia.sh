...
arch-chroot /mnt /bin/bash -c "pacman --noconfirm -S linux-headers"
# Install Nvidia Support
arch-chroot /mnt /bin/bash -c "pacman --noconfirm -S cuda nvidia nvidia-libgl nvidia-utils opencl-nvidia freeglut glu libxmu mesa opengl-man-pages cuda-devel nvidia-docker"
cat > /mnt/etc/modprobe.d/ec2-nvidia.conf <<- "EOF"
	...
	# Possible values:
	#
	# 1 - perform PCI configuration space checks (default)
	# 0 - do not perform PCI configuration space checks
...
cat >> /mnt/etc/rc.local << "EOF"
# Improves CUDA initialization speed. Comparison:
#
# Without persistent mode:
# [root@ip-10-16-16-180 ~]# time nvidia-smi -q
#  ...
# real    0m3.233s
# user    0m0.000s
# sys     0m2.470s
